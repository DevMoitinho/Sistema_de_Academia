    ...
    try:
        if tipo == 1:
            try:
                ...
                banco.commit()
                banco.close()
                input("Aluno removido com sucesso.")
            except sqlite3.Error as erro:
                input(f"Houve um erro ao remover o aluno.\nErro: '{erro}'.")
        ...
        input('Houve um erro ao tentar excluir o aluno.')
...
def updateAluno():
    ...
    if mat in usuarios:
        ...
        email = TRATAMENTOS.tratarStrings(input("Insira o e-mail do aluno: ").lower())
        email = TRATAMENTOS.processar_email(email)
        cursor.execute(("UPDATE Alunos SET Nome=?, Email=? WHERE Matricula=?"), (nome, email, mat,))
...
def updateSenha(mat):
    ...
    cursor.execute(("UPDATE Alunos SET Senha=? WHERE Matricula=?"), (TRATAMENTOS.criptografador(senha_nova), mat,))
    banco.commit()
    banco.close()
